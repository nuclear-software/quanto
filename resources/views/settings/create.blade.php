    ...
    <script type="text/javascript">
        ...
        var section = new Vue({
            ...
            data: {
            	...
            	@foreach($create['columns'] as $column){!!$column['name']!!}:@if( !($column['element']=='json' || $column['element']=='images_upload'))`{!! '{!'.'!'."old('".$column['name']."')".'!'.'!}'!!}`@else {!! '{!'.'!'."json_encode(old('".$column['name']."')?:[])".'!'.'!}'!!} @endif @if (!$loop->last),@endif

                @endforeach

            },
            methods: {
            	@foreach($create['columns'] as $column)@if ($column['element']=='json')add{!! up_camel_case($column['name']) !!}(type){
                    elements= {
